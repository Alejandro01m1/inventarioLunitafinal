...
if (formFinanzas) {
    formFinanzas.addEventListener("submit", async (e) => {
        ...
        const reinversion = Number(document.getElementById("reinversion").value || 0);
...
async function cargarDatos() {
    ...
    finSnap.forEach(doc => {
        ...
        tReinversion += Number(d.reinversion || 0);
    });

    // Cálculos
    const dineroReal = tVendido - tCostoVendido - tEnvio - tDeben - tReinversion;
    ...
    document.getElementById("totalReinversion").innerText =
        "$" + tReinversion.toLocaleString();

    document.getElementById("dineroReal").innerText =
        "$" + Math.round(dineroReal).toLocaleString();

    console.log("--- RESULTADOS FINALES ---");
    console.log("Suma total de Costos:", tCostoVendido);
    console.log("Reinversión:", tReinversion);
